...
const linkClass =
  'rounded-full bg-btn-primary px-4 py-2 text-sm font-medium text-foreground-soft transition-colors hover:bg-btn-primary-hover min-h-[44px] inline-flex items-center justify-center';
  ...
  return (
    <header className="relative flex min-h-[88px] items-center justify-between rounded-2xl bg-card-bg px-3 py-4 shadow-(--shadow-soft) sm:px-4 sm:py-5">
      <div className="flex min-w-0 flex-1 items-center gap-2">
        <AppLogo size="header" />
        <h1 className="hidden truncate text-2xl font-semibold tracking-tight text-foreground-soft sm:block">
          POTS Tracker
        </h1>
      </div>
      ...
      <button
        ...
        className="flex min-h-[44px] min-w-[44px] shrink-0 items-center justify-center rounded-full bg-card-bg shadow-(--shadow-soft) transition-colors hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-pastel-outline-pink/50 md:hidden"
      >
        {menuOpen ? (
          <svg
      ...
      {menuOpen && (
        <>
          ...
          />
          <div className="absolute right-3 top-full z-50 mt-2 flex w-[min(calc(100vw-1.5rem),16rem)] flex-col gap-2 rounded-2xl bg-card-bg p-3 shadow-(--shadow-soft) md:hidden">
            <ThemeToggle />
            {links.map(({ href, label }) => (
              <Link
                key={href}
